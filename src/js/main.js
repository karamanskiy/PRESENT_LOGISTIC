...
$(function(){
	...
	$(document).ready(function(e){
		...
		if(window.outerWidth == 1920 && (window.outerHeight < 1100 && window.outerHeight > 1000)) {
			// $('#bl1_s3 #finger-left').css('transform', 'translate(-82%, 6%)');
			// $('#bl1_s3 #finger-right').css('transform', 'translate(43%, -23%)');
			$('.secondStepWrap').css('top', '35%');
			$('#sect6 .bigCar, #sect7 .bigCar, #sect9 .bigCar').css('bottom', '8%');
			$s7cran = '9%';
			$bigCar_s7_perem = '61.5%';
			$('#s7wrap .bigCran, #s9wrap .bigCran').css('height', '62%');
			$('#s7wrap .mainSostav').css('width', '127%');
			$sostavContainerPerem = '629%';
			$sostavContainerPerem2Y = '-30.7%';
			...
			$kursPerem2 = '43%'
			$('.secondStepWrap.active #s4btn2').css('top', '17%');
			$cont7perem = '79%';
			$('#s7wrap .sostavContainer').css({'left':'48%', 'bottom': '28%'});
			$mainSostavPerem = '-58%';
		}
